...
description: !translate Aplicar GitHub Copilot CLI a flujos de trabajo de desarrollo cotidianos, incluyendo revisión de código, refactorización, depuración, generación de pruebas y Git.
...
![Chapter 03: Flujos de trabajo de desarrollo](../../../03-development-workflows/assets/chapter-header.png)

> **¿Y si la IA pudiera encontrar errores que ni siquiera sabías que debías preguntar?**
...
Un carpintero no solo sabe cómo usar herramientas, también tiene *flujos de trabajo* para diferentes tareas:

<img src="../../../03-development-workflows/assets/carpenter-workflow-steps.png" alt="Taller de artesano que muestra tres carriles de flujo de trabajo: Fabricación de muebles (Medir, Cortar, Ensamblar, Acabar), Reparación de daños (Evaluar, Retirar, Reparar, Emparejar), y Control de calidad (Inspeccionar, Probar uniones, Comprobar alineación)" width="800"/>

De manera similar, los desarrolladores tienen flujos de trabajo para distintas tareas. GitHub Copilot CLI mejora cada uno de estos flujos de trabajo, haciéndote más eficiente y efectivo en tus tareas de programación diarias.

---

# Los cinco flujos de trabajo

<img src="../../../03-development-workflows/assets/five-workflows.png" alt="Cinco iconos brillantes de neón que representan los flujos de trabajo de revisión de código, pruebas, depuración, refactorización e integración con Git" width="800"/>

Cada flujo de trabajo a continuación es independiente. Elige los que coincidan con tus necesidades actuales o recorre todos ellos.

---

## Elige tu propia aventura

Este capítulo cubre cinco flujos de trabajo que los desarrolladores suelen usar. **¡Sin embargo, no necesitas leerlos todos de una vez!** Cada flujo de trabajo es independiente y está en una sección plegable más abajo. Elige los que se ajusten a lo que necesitas y a tu proyecto actual. Siempre puedes volver y explorar los demás más tarde.
...
| Rastrear y corregir un error | [Flujo de trabajo 3: Depuración](#workflow-3-debugging) |
...
Pide a Copilot CLI que enfoque su revisión en una preocupación específica (aquí, la validación de entrada) enumerando las categorías que te interesan en el prompt.
...
Referencia un directorio completo con `@` para permitir que Copilot CLI escanee todos los archivos del proyecto a la vez.
...
El comando `/review` invoca el **agente de revisión de código** integrado, que está optimizado para analizar cambios preparados y no preparados con una alta relación señal-ruido. Usa un comando con barra para activar un agente integrado especializado en lugar de escribir un prompt libre.
...
<img src="../../../03-development-workflows/assets/refactoring-swimlane-single.png" alt="Flujo de refactorización: evaluar código, planear cambios, implementar, verificar comportamiento." width="800"/>
...
Comienza con mejoras sencillas. Prueba estas en la aplicación de libros. Cada prompt usa una referencia de archivo con `@` emparejada con una instrucción de refactorización específica para que Copilot CLI sepa exactamente qué cambiar.
...
> 💡 **¿Nuevo en la refactorización?** Empieza con solicitudes simples como añadir anotaciones de tipos o mejorar los nombres de variables antes de abordar transformaciones complejas.
...
Referencia múltiples archivos con `@` en un único prompt para que Copilot CLI pueda mover código entre ellos como parte de la refactorización.
...
Proporciona dos archivos relacionados y describe la preocupación transversal para que Copilot CLI pueda sugerir una solución coherente para ambos.
...
Encadena dos solicitudes relacionadas en una conversación de múltiples turnos. Primero genera pruebas y luego refactoriza con esas pruebas como red de seguridad.
...
Comienza describiendo qué está mal. Aquí hay patrones comunes de depuración que puedes probar con la aplicación de libros con errores. Cada prompt empareja una referencia de archivo `@` con una descripción clara del síntoma para que Copilot CLI pueda localizar y diagnosticar el error.
...
Aquí es donde brilla la depuración con conciencia de contexto. Prueba este escenario con la aplicación de libros con errores. Proporciona el archivo completo vía `@` y describe solo el síntoma reportado por el usuario. Copilot CLI rastreará la causa raíz y puede detectar errores adicionales cercanos.
...
**Por qué esto importa**: Copilot CLI lee todo el archivo, entiende el contexto de tu informe de errores y te da una solución específica con una explicación clara.

> 💡 **Extra**: Debido a que Copilot CLI analiza todo el archivo, a menudo descubre *otros* problemas que no pediste. Por ejemplo, al arreglar la búsqueda por autor, ¡Copilot CLI también podría notar el error de sensibilidad a mayúsculas y minúsculas en `find_book_by_title`!

### Nota de seguridad del mundo real

Aunque depurar tu propio código es importante, entender las vulnerabilidades de seguridad en aplicaciones de producción es crítico. Prueba este ejemplo: Apunta Copilot CLI a un archivo desconocido y pídele que audite en busca de problemas de seguridad.
...
> - **Inyección SQL**: Cuando la entrada del usuario se pone directamente en una consulta de base de datos, permitiendo a atacantes ejecutar comandos maliciosos
...
Describe la entrada exacta y la salida observada para darle a Copilot CLI un caso de prueba concreto y reproducible con el que razonar.
...
Al escribir pruebas manualmente, los desarrolladores suelen crear 2-3 pruebas básicas:
- Probar entrada válida
- Probar entrada inválida
- Probar un caso límite

Mira lo que sucede cuando le pides a Copilot CLI que genere pruebas completas. Este prompt usa una lista estructurada con viñetas y una referencia de archivo `@` para guiar a Copilot CLI hacia una cobertura de pruebas exhaustiva:
...
**Qué obtienes**: 15+ pruebas completas que incluyen:
...
Apunta a una única función y enumera las categorías de entrada que quieres probar para que Copilot CLI genere pruebas unitarias focalizadas y exhaustivas.
...
Solicita pruebas *adicionales* para una única función para que Copilot CLI genere nuevos casos que complementen lo que ya tienes.
...
<img src="../../../03-development-workflows/assets/git-integration-swimlane-single.png" alt="Flujo de integración con Git: preparar cambios, generar mensaje, hacer commit, crear PR." width="800"/>
...
> **Prueba esto primero:** `copilot -p "Generate a conventional commit message for: $(git diff --staged)"` — añade algunos cambios al área de staging y luego ejecuta esto para ver a Copilot CLI escribir tu mensaje de commit.

Este ejemplo usa la bandera de aviso inline `-p` con sustitución de comandos de shell para pasar la salida de `git diff` directamente a Copilot CLI para un mensaje de commit de una sola vez. La sintaxis `$(...)` ejecuta el comando dentro de los paréntesis e inserta su salida en el comando exterior.
...
# Generar un mensaje de commit usando el formato [Conventional Commit](../GLOSSARY.md#conventional-commit)
...
*Los resultados de la demostración varían. Tu modelo, herramientas y respuestas diferirán de lo mostrado aquí.*
...
Pasa la salida de `git show` a un prompt `-p` para obtener un resumen en inglés claro del último commit.
...
Si estás trabajando con una rama en el modo interactivo de Copilot CLI, puedes usar el comando `/pr` para trabajar con pull requests. Usa `/pr` para ver un PR, crear un nuevo PR, arreglar un PR existente o dejar que Copilot CLI decida automáticamente según el estado de la rama.
...
Usa `git diff main..HEAD` dentro de un prompt `-p` para una comprobación rápida de coherencia antes del push en todos los cambios de la rama.
...
# 2. Abre un pull request como borrador
...
El comando `/diff` muestra todos los cambios realizados durante tu sesión actual. Usa este comando slash para ver un diff visual de todo lo que Copilot CLI ha modificado antes de que hagas commit. También funciona en carpetas que no son repositorios git.
...
A veces quieres explorar dos enfoques diferentes a un problema sin perder tu conversación original. El comando `/branch` (también disponible como `/fork`) crea una copia de tu sesión actual para que puedas probar una dirección diferente y luego comparar resultados.
...
Copilot busca en repositorios de GitHub y fuentes web, y luego devuelve un resumen con referencias. Esto es útil cuando vas a comenzar una nueva característica y quieres tomar decisiones informadas primero. Puedes compartir los resultados usando `/share`.
...
Tras completar las demostraciones, prueba estas variaciones:

1. **Desafío Detective de errores**: Pide a Copilot CLI que depure la función `mark_as_read` en `samples/book-app-buggy/books_buggy.py`. ¿Explicó por qué la función marca TODOS los libros como leídos en lugar de solo uno?
...
3. **Desafío de mensaje de commit**: Realiza cualquier pequeño cambio en un archivo de la aplicación de libros, añádelo al área de staging (`git add .`), luego ejecuta:
   ```bash
   copilot -p "Generate a conventional commit message for: $(git diff --staged)"
   ```
   ¿Es el mensaje mejor de lo que habrías escrito rápidamente?

**Autoevaluación**: Entiendes los flujos de trabajo de desarrollo cuando puedes explicar por qué "debug this bug" es más poderoso que "find bugs" (¡el contexto importa!).
...
1. **Revisión**: Pide a Copilot CLI que revise `remove_book()` en `books.py` en busca de casos límite y problemas potenciales:
...
4. **Revisión**: Añade tus cambios al área de staging y ejecuta `/review` para comprobar si quedan problemas
...
**Consejo:** Después de mejorar `remove_book()`, intenta preguntarle a Copilot CLI: "Are there any other functions in this file that could benefit from the same improvements?". Puede sugerir cambios similares a `find_book_by_title()` o `find_by_author()`.
...
El ejercicio muestra a los desarrolladores cómo usar GitHub Copilot CLI para crear issues, generar código y colaborar desde la terminal mientras construyen una app calculadora en Node.js. Instalarás la CLI, usarás plantillas y agentes, y practicarás desarrollo iterativo impulsado desde la línea de comandos.

##### <img src="../../../assets/github-skills-logo.png" width="28" align="center" /> [Comienza el ejercicio "Crear aplicaciones con Copilot CLI" de Skills](https://github.com/skills/create-applications-with-the-copilot-cli)
...
|---------|--------------|-----|
| Usar prompts vagos como "Review this code" | Retroalimentación genérica que no detecta problemas específicos | Sé específico: "Review for SQL injection, XSS, and auth issues" |
| No usar `/review` para revisiones de código | Falta el agente optimizado para revisión de código | Usa `/review` que está ajustado para una salida con alta relación señal/ruido |
...
| Generar pruebas sin especificar el framework | Las pruebas pueden usar sintaxis o librería de aserciones incorrecta | Especifica: "Generate tests using Jest" o "using pytest" |
...
|-------|---------|----------------|
| Comandos básicos | Ch 01 | Usar el modo interactivo, modo plan, modo programático (-p), y comandos slash |
...
Los capítulos 04-06 cubren características adicionales que añaden aún más capacidades y vale la pena aprender.
...
- **Guarda los prompts que funcionen bien.** Cuando Copilot CLI cometa un error, anota qué salió mal. Con el tiempo, esto se convertirá en tu manual personal.
...
Los capítulos restantes cubren funciones adicionales que extienden las capacidades de Copilot CLI:

| Capítulo | Qué cubre | Cuándo lo querrás |
|---------|----------------|---------------------|
| Ch 04: Agentes | Crear personas de IA especializadas | Cuando quieras expertos de dominio (frontend, security) |
| Ch 05: Skills | Cargar automáticamente instrucciones para tareas | Cuando repitas los mismos prompts a menudo |
...
- Creación de agentes especializados (experto frontend, auditor de seguridad) con archivos `.agent.md`
- Patrones de colaboración multiagente
- Archivos de instrucciones personalizadas para los estándares del proyecto
